...
class Task {
	...
	private:
		...
		// adj[i] = list of nodes adjacent to node i
		vector<int> adj[NMAX];
		// data_set_needed[i] = 1 if task i needs data
		// set 1, 2 if task i needs data set 2
		...
		void add_nodes_with_in_degree0(vector<int> &in_degree,
		...
		void update_queue(deque<pair<int, int>> &q,
						  vector<int> &in_degree, int task) {
			// Add the nodes that have in_degree = 0 after we remove a node
			for (auto &i : adj[task]) {
				// We remove a node, so we decrease the in_degree of the
				...
				if (in_degree[i] == 0) {
					...
					if (data_set_needed[i] == 1) {
						...
						// Those with data_set_needed = 2 go in the back
						q.push_back({i, data_set_needed[i]});
					}
				}
			}
		...
		void start_with_first_dataset(deque<pair<int, int>> &q,
			...
			while (!q.empty()) {
				...
				// If we don't have a last task, we take the first task
				// from the queue so it needs the first data set
				if (!last_data_set_needed) {
					task = q.front().first;
					set_needed = q.front().second;
					q.pop_front();
				} else if (last_data_set_needed == 1) {
					// If we have a last task, we take the task from the front
					// of the queue that uses the same dataset as the last task
					task = q.front().first;
					set_needed = q.front().second;
					q.pop_front();
					...
					// If we have a last task, we take the task from the back
					// of the queue that needs
		...
		void start_with_second_dataset(deque<pair<int, int>> &q_copy,
									   vector<int> &in_degree_copy,
									   int &min_switches2) {
			int last_data_set_needed = 0;
			...
			while (!q_copy.empty()) {
				...
				// If we don't have a last task, we take the last task
				// from the queue so it needs
				// the second data set
				if (!last_data_set_needed) {
					...
					// If we have a last task, we take the task from the
					// front of the queue that needs
					// the same data set as the last task
		...
		int get_result() {
			...
			// Start with the first data set and compute the minimum
			// number of switches
			start_with_first_dataset(q, in_degree, min_switches1);

			// Start with the second data set and compute the minimum
			// number of switches
