...
class Task {
	...
	private:
		...
		vector<int> adj[NMAX];
		// dfs[i] = i-th node in dfs traversal
		vector<int> dfs;
		// queries[i] = i-th query
		vector<pair<int, int>> queries;
		// visited[i] = true if node i was visited in dfs
		vector<bool> visited;
		// number_of_children[i] = number of children of node i in dfs
		...
		vector<int> get_result() {
			...
			// pos[i] = position of node i in dfs traversal
			for (long unsigned int i = 0; i < dfs.size(); i++) {
				pos[dfs[i]] = i;
			}
			...
			for (int i = 0; i < nr_queries; i++) {
				...
				// If we make more than `number_of_children[src]`
				// steps then we will get out of
				// the subtree of `src`, so the destination will be -1
				int dest = steps > number_of_children[src] ? -1 :
								   dfs[pos[src] + steps];
